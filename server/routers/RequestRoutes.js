...
// Helper function to calculate request status based on item statuses
...
// Helper function to handle item reassignment
async function handleItemReassignment(itemId, newReceiverId, originalRequestId, currentUserId, reason = '') {
    try {
        ...
        console.log(`Item ${itemId} reassigned from ${currentUserId} to ${newReceiverId}`);
...
requestRouter.post("/add", async (req, res) => {
    ...
    if (req.user.role !== "end_user") {
        return res.status(403).json({ message: "Forbidden: You are not allowed to create requests" });
    }
        ...
        const itemIds = await Promise.all(
            items.map(async (item, idx) => {
                ...
                if (!name || !type || typeof quantity !== 'number') {
                    throw new Error(`Item at index ${idx} missing required fields (name, type, quantity)`);
                }
...
requestRouter.post("/update", async (req, res) => {
    try {
        ...
        // Check permissions - either the request creator or receiver can update
        if (req.user.role === "end_user" && existingRequest.user.toString() !== req.user._id.toString()) {
            return res.status(403).json({ message: "Forbidden: You can only update your own requests" });
        }
        if (req.user.role === "receiver" && existingRequest.receiver.toString() !== req.user._id.toString()) {
            return res.status(403).json({ message: "Forbidden: You can only update requests assigned to you" });
        }

        // Handle item updates and reassignments
        if (items && Array.isArray(items)) {
            for (const itemData of items) {
                if (itemData.id && mongoose.Types.ObjectId.isValid(itemData.id)) {
                    const currentItem = existingRequest.items.find(item => item._id.toString() === itemData.id);
        ...
        // Update the request with calculated status
...
// Get reassignments for a user
requestRouter.get("/reassignments", async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ message: "Unauthorized" });
        }

        const reassignments = req.user.role === "receiver" 
            ? await Reassignment.find({ reassignedTo: req.user._id })
                .populate('originalRequest')
                .populate('item')
                .populate('reassignedFrom', 'username')
                .populate('reassignedTo', 'username')
            : await Reassignment.find({ reassignedFrom: req.user._id })
                .populate('originalRequest')
                .populate('item')
                .populate('reassignedFrom', 'username')
                .populate('reassignedTo', 'username');
        ...
        res.status(200).json({ 
            message: "Reassignments fetched successfully", 
